...
class Data_buku_tamu extends CI_Controller
{
	...
	function get_kartu()
	{
		...
		foreach ($result_kartu as $row) {
			if ($row->status = 'y') {
				$status = "<button class='btn btn-xs btn-danger'> Belum Di kembalikan</button>";
			} else {
				$status = "<button class='btn btn-xs btn-default'> Selesai</button>";
			}
			$hasil .= "<li>" . substr($row->serial_kartu, 0, 15) . "...<br>" . $status . "</li>";
			$no_kartu++;
		}
		$hasil .= "</ul>";
		$data['hasil'] = $hasil;
